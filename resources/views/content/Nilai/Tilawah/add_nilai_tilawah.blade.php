            ...
            <div class="card">
                <div class="row">
                    ...
                    <div class="col-4" style="margin-top: 20px">
                        <select class="select2-size-sm form-select" name="tilawah_id" id="tilawah_id">
                            <option selected disabled>Pilih Tilawah</option>
                ...
                <div class="card-datatable">
                    <form id="jquery-val-form" action="{{route('nilai.tilawah.add')}}" method="post" enctype="multipart/form-data">
                        {{csrf_field()}}
                            ...
                            <tbody>
                                @foreach ($murid as $m)
                                    <tr>
                                        ...
                                        {{-- ID NILAI --}}
                                            <input class="id_nilai_tilawah" type="hidden" name="id_nilai_tilawah[{{$m->id}}]" id="id_nilai_tilawah_{{$m->id}}">
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    ...
    // SELECT INDIKATOR IBADAH
        $('#tilawah_id').change(function () {
            var id_indicators = document.getElementById("tilawah_id").value;
    ...
    // RESET TABEL
        const data_edit = function(id_indicators, id_pengajar){
            if (id_indicators, id_pengajar) {
                $.ajax({
                    type: "GET",
                    url: "/Nilai/Tilawah/edit_nilai?id_indicators=" +id_indicators+"&id_pengajar="+id_pengajar,
                    dataType: 'JSON',
                    ...
                    error:function(result)
                    {
                        const reset = (idx, elem) => (elem.value = null)
                            ...
                            $('.id_nilai_tilawah').map(reset).change()
                            $('.predikat').val("").change();
                    }
                });
            } else {
