...
@section('title', 'Input Doa Harian')
            ...
            <div class="card">
                <div class="row">
                    ...
                    <div class="col-4" style="margin-top: 20px">
                        <select class="select2-size-sm form-select" name="doa_id" id="doa_id">
                            <option selected disabled>Pilih Doa</option>
                            @foreach ($doa as $d)
                                <option value="{{$d->id}}">{{$d->indicators}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="card-datatable">
                    <form id="jquery-val-form" action="{{route('nilai.doa.add')}}" method="post" enctype="multipart/form-data">
                        {{csrf_field()}}
                            ...
                            <tbody>
                                @foreach ($murid as $m)
                                    <tr>
                                        ...
                                        {{-- ID NILAI --}}
                                            <input class="id_nilai_doa" type="hidden" name="id_nilai_doa[{{$m->id}}]" id="id_nilai_doa_{{$m->id}}">
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    ...
    // SELECT INDIKATOR IBADAH
        $('#doa_id').change(function () {
            var id_indicators = document.getElementById("doa_id").value;
    ...
    // RESET TABEL
        const data_edit = function(id_indicators, id_pengajar){
            if (id_indicators, id_pengajar) {
                $.ajax({
                    type: "GET",
                    url: "/Nilai/Doa/edit_nilai?id_indicators=" +id_indicators+"&id_pengajar="+id_pengajar,
                    dataType: 'JSON',
