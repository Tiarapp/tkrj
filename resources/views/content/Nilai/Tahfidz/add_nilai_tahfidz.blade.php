...
@section('title', 'Input Nilai Tahfidz')
            ...
            <div class="card">
                <div class="row">
                    ...
                    <div class="col-4" style="margin-top: 20px">
                        <select class="select2-size-sm form-select" name="tahfidz_id" id="tahfidz_id">
                            <option selected disabled>Pilih Tahfidz</option>
                            @foreach ($tahfidz as $t)
                                <option value="{{$t->id}}">{{$t->indicators}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="card-datatable">
                    <form id="jquery-val-form" action="{{route('nilai.tahfidz.add')}}" method="post" enctype="multipart/form-data">
                        {{csrf_field()}}
                            ...
                            <tbody>
                                @foreach ($murid as $m)
                                    <tr>
                                        ...
                                        {{-- ID NILAI --}}
                                            <input class="id_nilai_tahfidz" type="hidden" name="id_nilai_tahfidz[{{$m->id}}]" id="id_nilai_tahfidz_{{$m->id}}">
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    ...
    // SELECT INDIKATOR IBADAH
        $('#tahfidz_id').change(function () {
            var id_indicators = document.getElementById("tahfidz_id").value;
    ...
    // RESET TABEL
        const data_edit = function(id_indicators, id_pengajar){
            if (id_indicators, id_pengajar) {
                $.ajax({
                    type: "GET",
                    url: "/Nilai/Tahfidz/edit_nilai?id_indicators=" +id_indicators+"&id_pengajar="+id_pengajar,
                    dataType: 'JSON',
                    success:function(result){
                        ...
                        if (result.length == 0) {
                            ...
                            $('.id_nilai_tahfidz').map(reset)
                            $('.predikat').val("").change();
                        } else {
                            for (let index = 0; index < result.length; index++) {
                                $("#id_nilai_tahfidz_"+result[index].murid_id).val(result[index].id);
                                $("#predikat_id_"+result[index].murid_id).val(result[index].nilai).change();
                                document.getElementById("hreff_"+result[index].murid_id).href="/Nilai/Tahfidz/delete"+result[index].id;
                    ...
                    error:function(result)
                    {
                        const reset = (idx, elem) => (elem.value = null)
                            ...
                            $('.id_nilai_tahfidz').map(reset).change()
                            $('.predikat').val("").change();
                    }
                });
            } else {
